...
function SectionPrintFarm({ printers = [], jobs = [] }) {
  return (
    <Widget title="3D Ферма — принтери та роботи" sub="Стани обладнання і прогрес завдань">
      <div className="list">
        {printers.map((p) => (
          <div key={p.id} className="row row--tight">
            <strong>{p.name}</strong>
            <span className="small">
              {p.model} • {p.nozzle || "n/a"}
            </span>
            <Pill
              text={p.state || "unknown"}
              tone={p.state === "printing" ? "ok" : p.state === "error" ? "danger" : "info"}
            />
        ...
        <tbody>
          {rows.map(([name, ready, running, lag]) => (
            <tr key={name}>
              ...
              <td>{running}</td>
              <td>
                <Pill tone={tone(lag)} text={`${lag} ms`} />
              </td>
            </tr>
          ))}
        </tbody>
...
function SectionPayments({ p = {} }) {
  return (
    <Widget title="Оплати" sub="Передоплата та доплати перед відвантаженням">
      <div className="kpi">
        <div className="box">
          <div className="small">Очікує 25%</div>
          <div className="num">{p.awaitingPrepay || 0}</div>
        </div>
        <div className="box">
          <div className="small">Очікує доплату</div>
          <div className="num">{p.awaitingRest || 0}</div>
        </div>
        <div className="box">
          <div className="small">Disputes</div>
          <div className="num">{p.disputes || 0}</div>
        </div>
        <div className="box">
          <div className="small">Середній чек</div>
          <div className="num">{p.avgCheckUAH || 0} ₴</div>
        </div>
      </div>
    </Widget>
  );
...
function SectionServices({ s = {} }) {
  const asPill = (v) =>
    v === "up" ? (
      <Pill text="UP" tone="ok" />
    ) : v === "degraded" ? (
      <Pill text="DEGRADED" tone="warn" />
    ) : (
    ...
    <Widget title="Пошуковий індекс" sub="Беклог індексації каталогу">
      <div className="kpi">
        <div className="box">
          ...
          <div className="num">{idx.backlog || 0}</div>
        </div>
        <div className="box">
          <div className="small">Оновлено</div>
          <div className="num">{idx.lastIndexedAt || "—"}</div>
        </div>
        <div className="box">
          <div className="small">Швидкість</div>
          <div className="num">{idx.ratePerMin || 0}/m</div>
        </div>
        <div className="box">
          <div className="small">Шард</div>
          <div className="num">{idx.shards || 1}</div>
        </div>
      </div>
    </Widget>
  );
...
function SectionIngester({ ing = {} }) {
  ...
  return (
    <Widget title="Імпорт/Ingester" sub="CSV, медіа, нормалізація">
      ...
      <div className="kpi">
        <div className="box">
          <div className="small">Media backlog</div>
          <div className="num">{ing.mediaBacklog || 0}</div>
        </div>
        <div className="box">
          <div className="small">Трансформації/хв</div>
          <div className="num">{ing.mediaRatePerMin || 0}</div>
        </div>
        <div className="box">
          <div className="small">Помилки 1h</div>
          <div className="num">{ing.errors1h || 0}</div>
        </div>
        <div className="box">
          <div className="small">Версія pricing.yml</div>
          <div className="num">{ing.pricingVersion || "—"}</div>
        </div>
      </div>
    </Widget>
  );
        ...
        <tbody>
          {items.map(([name, v]) => (
            <tr key={name}>
              ...
              <td>
                <Pill tone={v.failed5m > 0 ? "warn" : "ok"} text={String(v.failed5m || 0)} />
              </td>
...
export default function Board() {
  ...
  // SSE обновления (используем твой общий хук)
    ...
    onEvent: (e) => {
      if (e.type === "print.progress") {
        setPrints((cur) => ({
          ...cur,
          jobs: cur.jobs.map((j) =>
            j.id === e.entity_id ? { ...j, progress: e.data.progress, eta: e.data.eta } : j
          ),
        }));
      }
      if (e.type === "printer.state") {
        setPrints((cur) => ({
          ...cur,
          printers: cur.printers.map((p) => (p.id === e.entity_id ? { ...p, state: e.data.state } : p)),
        }));
      }
      if (e.domain === "ops") {
        setOps((cur) => ({ ...cur, ...e.payload }));
      }
    },
  });
