...
function mergeDefaults(defaults, incoming) {
  // простая рекурсивная “накладка” входящих поверх дефолтов
...
function ChipsEditor({ value, onChange, placeholder = "Введи и нажми Enter" }) {
...
function DangerZone({ title, children }) {
  return (
    <div style={{ border: "1px solid var(--danger-border)", borderRadius: 12, padding: 12, background: "color-mix(in srgb, var(--secondary) 6%, var(--surface))" }}>
...
export default function Settings() {
  ...
  const nav = useMemo(
    () => [
      { id: "ui", title: "1) Общие (UI/Wallboard)" },
      { id: "backups", title: "2) Backups & Retention" },
      { id: "infra", title: "3) Инфраструктура (Servers/Topology)" },
      { id: "ops", title: "4) Services Health & Ops actions" },
      { id: "queues", title: "5) Очереди и воркеры (Queues)" },
      { id: "catalog", title: "6) Каталог 3M SKU: Indexer / Ingester / Import" },
      { id: "printFarm", title: "7) Производство/Print Farm" },
      { id: "logisticsPayments", title: "8) Логистика и оплаты" },
      { id: "webhooks", title: "9) Webhooks" },
      { id: "alerts", title: "10) Alerts & Notifications" },
      { id: "security", title: "11) Security & Access" },
    ],
    []
  );
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(() => {
      try {
        ...
        setToast({ kind: "success", text: "Збережено локально ✅" });
        setTimeout(() => setToast(null), 1200);
      } catch {
        setToast({ kind: "error", text: "Не вдалося зберегти (localStorage)." });
      }
    }, 350);
  ...
  const importJson = async (file) => {
    ...
    if (!parsed) {
      setToast({ kind: "error", text: "Файл не схожий на JSON." });
      return;
    }
    setCfg(mergeDefaults(DEFAULTS, parsed));
    setToast({ kind: "success", text: "Імпортовано ✅" });
  ...
  const doAction = async ({ title, description, url, body }) => {
    ...
    try {
      ...
      setToast({ kind: "success", text: `${title}: OK` });
    } catch (e) {
      setToast({ kind: "error", text: `${title}: ${String(e.message || e)}` });
    } finally {
    ...
    <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: 16, alignItems: "start" }}>
      ...
      <div className="card" style={{ position: "sticky", top: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
          <button className="buttonSecondary" type="button" onClick={exportJson}>Експорт JSON</button>
          ...
          <button type="button" onClick={resetAll}>Скинути</button>
        </div>
        ...
        <div style={{ display: "grid", gap: 6 }}>
          {nav.map((x) => (
            <a key={x.id} href={`#${x.id}`} style={{ padding: "6px 8px", borderRadius: 10, background: "color-mix(in srgb, var(--primary) 3%, transparent)" }}>
              {x.title}
            </a>
          ))}
        </div>
      ...
      <div>
        {/* 1) UI */}
        <div id="ui" />
        <Card title="1) Общие (UI/Wallboard)" sub="SSE/polling, тумблери секцій, пороги підсвітки, локаль/час/валюта">
          <FieldRow
            label="SSE (Server-Sent Events)"
            hint="Якщо увімкнено — Board може отримувати оновлення без polling."
          >
            <div style={{ display: "grid", gap: 10 }}>
              <Toggle value={cfg.ui.sseEnabled} onChange={(v) => patch("ui.sseEnabled", v)} label="Увімкнути SSE" />
              <div>
                <div className="muted" style={{ fontSize: 12, marginBottom: 6 }}>Topics (orders, prints, shipments, ops)</div>
                <ChipsEditor
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                ...
                <div>
                  <div className="muted" style={{ fontSize: 12 }}>Дедуплікація (ms)</div>
          ...
          <FieldRow label="Polling" hint="Як запасний режим, якщо SSE недоступний.">
            <div style={{ display: "grid", gap: 10 }}>
              <Toggle value={cfg.ui.pollingEnabled} onChange={(v) => patch("ui.pollingEnabled", v)} label="Увімкнути polling" />
              <div style={{ maxWidth: 320 }}>
                <div className="muted" style={{ fontSize: 12 }}>Інтервал (ms)</div>
          ...
          <FieldRow label="Пороги підсвітки (warn/danger)" hint="Використовуються для KPI, lag, backlog, низьких запасів.">
            <div style={{ display: "grid", gap: 12 }}>
              <div className="card" style={{ padding: 12 }}>
                <div style={{ fontWeight: 800, marginBottom: 8 }}>Lag черг (ms)</div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  <div>
              ...
              <div className="card" style={{ padding: 12 }}>
                <div style={{ fontWeight: 800, marginBottom: 8 }}>Низький запас матеріалу</div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  <div>
                    <div className="muted" style={{ fontSize: 12 }}>Filament warn (кг)</div>
                  ...
                  <div>
                    <div className="muted" style={{ fontSize: 12 }}>Resin warn (л)</div>
          ...
          <FieldRow label="Локаль: таймзона/час/валюта" hint="Форматування часу на Board та грошових значень у таблицях.">
            <div style={{ display: "grid", gap: 12, maxWidth: 520 }}>
              <div>
                <div className="muted" style={{ fontSize: 12 }}>Timezone</div>
                <TextInput value={cfg.ui.locale.timezone} onChange={(v) => patch("ui.locale.timezone", v)} placeholder="Europe/Kyiv" />
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  <div className="muted" style={{ fontSize: 12 }}>Time format</div>
                  <Select
                ...
                <div>
                  <div className="muted" style={{ fontSize: 12 }}>Currency</div>
                  <Select
        ...
        {/* 2) Backups */}
        <div id="backups" />
        <Card title="2) Backups & Retention" sub="Розклад, склад бекапу, ретеншн, сховище, ручні дії">
          ...
          <FieldRow label="Тип та склад бекапу" hint="Full/incremental + що саме бекапимо.">
            <div style={{ display: "grid", gap: 10 }}>
              <Select
          ...
          <FieldRow label="Вікно виконання" hint="Щоб не заважати піковим годinam.">
            <div style={{ display: "grid", gap: 10, maxWidth: 520 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                ...
                <div>
                  <div className="muted" style={{ fontSize: 12 }}>end</div>
          ...
          <FieldRow label="Сховище бекапів" hint="S3/MinIO/Filesystem. Ключі зазвичай зберігаються на сервері — тут лише профіль.">
            <div style={{ display: "grid", gap: 12, maxWidth: 620 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  <div className="muted" style={{ fontSize: 12 }}>bucket</div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  <div className="muted" style={{ fontSize: 12 }}>encryption profile</div>
                ...
                <div>
                  <div className="muted" style={{ fontSize: 12 }}>key profile</div>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <button
                ...
                onClick={() =>
                  doAction({
                    title: "Backup now",
                    description: "Запустити бекап негайно.",
        ...
        {/* 3) Infra */}
        <div id="infra" />
        <Card title="3) Инфраструктура (Servers/Topology)" sub="Ноди, ролі, пули/ліміти, maintenance mode">
          <FieldRow label="Список нод" hint="Ім'я, роль, хост. (UI-список; серверні дані підтягуються пізніше).">
            <div style={{ display: "grid", gap: 10 }}>
                  ...
                  <tbody>
                    {(cfg.infra.nodes || []).map((n, i) => (
                      <tr key={`${n.name}-${i}`}>
                        ...
                        <td>
                          <select value={n.role} onChange={(e) => {
                            const next = [...cfg.infra.nodes];
          ...
          <FieldRow label="Maintenance mode" hint="Обмежує функції, але може дозволити читання каталогу.">
            <div style={{ display: "grid", gap: 10 }}>
              <Toggle value={cfg.infra.maintenance.enabled} onChange={(v) => patch("infra.maintenance.enabled", v)} label="Увімкнути maintenance" />
        ...
        {/* 4) Ops */}
        <div id="ops" />
        <Card title="4) Services Health & Ops actions" sub="Пороги деградації + кнопки оператора (з підтвердженням)">
            ...
            <div style={{ display: "grid", gap: 10 }}>
              ...
              <DangerZone title="Небезпечна зона">
                <div className="muted" style={{ fontSize: 12, marginBottom: 8 }}>
                  Rebuild search index може зупинити пошук та навантажити інфраструктуру. Вмикай тільки якщо точно треба.
                </div>
                <button
                  type="button"
        ...
        {/* 5) Queues */}
        <div id="queues" />
        <Card title="5) Очереди и воркеры (Queues)" sub="Конкурентність, ретраї, DLQ, дедуп, батчі, таймаути">
          <FieldRow label="Пороги" hint="ready/running thresholds для підсвітки та алертів.">
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, maxWidth: 720 }}>
              <div>
          ...
          <FieldRow label="Retries / Backoff / DLQ" hint="Політика ретраїв і dead-letter queue.">
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, maxWidth: 720 }}>
              <div>
              ...
              <div style={{ display: "flex", alignItems: "center" }}>
                <Toggle value={cfg.queues.retries.dlqEnabled} onChange={(v) => patch("queues.retries.dlqEnabled", v)} label="DLQ enabled" />
              </div>
            </div>
          </FieldRow>
        ...
        {/* 7) Print farm */}
        <div id="printFarm" />
        <Card title="7) Производство/Print Farm" sub="Принтери, профілі, маршрутизація, SLA/таймаути">
          <FieldRow label="Принтери" hint="Маппінг принтерів, профілі сопел/матеріалів.">
            <div style={{ overflowX: "auto" }}>
              <table className="table">
                ...
                <tbody>
                  {(cfg.printFarm.printers || []).map((p, i) => (
                    <tr key={`${p.id}-${i}`}>
                      <td><input value={p.id} onChange={(e) => {
                        const next = [...cfg.printFarm.printers];
                        next[i] = { ...next[i], id: e.target.value };
                        patch("printFarm.printers", next);
                      }} /></td>
                      <td><input value={p.name} onChange={(e) => {
                        const next = [...cfg.printFarm.printers];
                        next[i] = { ...next[i], name: e.target.value };
                        patch("printFarm.printers", next);
                      }} /></td>
                      <td><input value={p.profile} onChange={(e) => {
                        const next = [...cfg.printFarm.printers];
                        next[i] = { ...next[i], profile: e.target.value };
                        patch("printFarm.printers", next);
                      }} /></td>
                      <td><input value={p.material} onChange={(e) => {
                        const next = [...cfg.printFarm.printers];
              ...
              <button
                type="button"
                onClick={() =>
                  patch("printFarm.printers", [...(cfg.printFarm.printers || []), { id: "new", name: "New printer", profile: "fdm-0.4", material: "PLA", enabled: true }])
                }
              >
                Додати принтер
              </button>
          ...
          <FieldRow label="Маршрутизація задач" hint="Прості правила: which printer-group for which material.">
            <div style={{ display: "grid", gap: 8 }}>
              <div className="muted" style={{ fontSize: 12 }}>rules</div>
              <ChipsEditor
          ...
          <FieldRow label="SLA / таймаути" hint="Автопауза при error, нотифікації простій/помилки.">
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, maxWidth: 720 }}>
              <div style={{ display: "flex", alignItems: "center" }}>
        ...
        <Card title="8) Логистика и оплаты" sub="Провайдери, ліміти, webhooks профілі, маппінг статусів, антифрод">
              ...
              {Object.entries(cfg.logisticsPayments.providers).map(([name, p]) => (
                <div key={name} className="card" style={{ padding: 12 }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                    ...
                    <Toggle value={p.enabled} onChange={(v) => patch(`logisticsPayments.providers.${name}.enabled`, v)} label="enabled" />
                  </div>
          ...
          <FieldRow label="Маппінг статусів" hint="Відповідність зовнішніх статусів внутрішнім.">
            <div style={{ maxWidth: 720 }}>
              <TextArea
                value={JSON.stringify(cfg.logisticsPayments.statusMapping, null, 2)}
                onChange={(v) => {
                  const parsed = safeParseJSON(v);
                  if (parsed) patch("logisticsPayments.statusMapping", parsed);
                }}
                rows={8}
                placeholder='{"shipment":{"created":"new"}}'
              />
              <div className="muted" style={{ fontSize: 12, marginTop: 6 }}>
                Порада: редагуй JSON обережно; якщо синтаксис зламається — зміни не застосуються.
              </div>
            </div>
          </FieldRow>
          ...
          <FieldRow label="Антифрод (акуратно)" hint="Пороги/правила перевірки замовлень.">
            <div style={{ display: "grid", gap: 10, maxWidth: 720 }}>
              <Toggle value={cfg.logisticsPayments.antifraud.enabled} onChange={(v) => patch("logisticsPayments.antifraud.enabled", v)} label="enabled" />
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                ...
                <div>
                  ...
                  <NumberInput value={cfg.logisticsPayments.antifraud.requireManualReviewAboveUAH} min={0} max={100000000} step={100} onChange={(v) => patch("logisticsPayments.antifraud.requireManualReviewAboveUAH", v)} />
                </div>
              </div>
            </div>
          </FieldRow>
        ...
        {/* 9) Webhooks */}
        <div id="webhooks" />
        <Card title="9) Webhooks" sub="Retries/backoff, ідемпотентність, підписи, останні помилки, тестовий event">
              ...
              <table className="table">
                ...
                <tbody>
                  {(cfg.webhooks.recentErrors || []).length ? (
                    ...
                    <tr>
                      <td colSpan="3" className="muted">Даних поки немає.</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          <FieldRow label="Send test event" hint="Опціонально: відправити тестовий вебхук (якщо API реалізовано).">
            <button
              type="button"
              onClick={() => doAction({
                title: "Send test event",
                description: "Надіслати тестовий event для перевірки траси.",
        ...
        {/* 10) Alerts */}
        <div id="alerts" />
        <Card title="10) Alerts & Notifications" sub="Канали, правила, quiet hours">
            ...
            <div style={{ display: "grid", gap: 12 }}>
              {Object.entries(cfg.alerts.channels).map(([name, c]) => (
                <div key={name} className="card" style={{ padding: 12 }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                    <div style={{ fontWeight: 800 }}>{name}</div>
                    <Toggle value={c.enabled} onChange={(v) => patch(`alerts.channels.${name}.enabled`, v)} label="enabled" />
                  </div>
          ...
          <FieldRow label="Правила" hint="Приклади: lag > X ms 5 хв, error rate, backlog, backup failed.">
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, maxWidth: 720 }}>
              <div>
          ...
          <FieldRow label="Тихі години" hint="Коли не турбувати (або тільки critical).">
            <div style={{ display: "grid", gap: 10, maxWidth: 520 }}>
              <Toggle value={cfg.alerts.quietHours.enabled} onChange={(v) => patch("alerts.quietHours.enabled", v)} label="enabled" />
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  <div className="muted" style={{ fontSize: 12 }}>start</div>
        ...
        {/* 11) Security */}
        <div id="security" />
        <Card title="11) Security & Access" sub="RBAC на дії, audit log змін">
